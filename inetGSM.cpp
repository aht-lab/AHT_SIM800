...
String InetGSM::httpGET(const char* server, int port, const char* path)
{
     ...
     */
     ...
     Serial.println("DB:DOC KQ");
     now = millis();
     while (millis() - now < apiTimeout) {
...
int InetGSM::httpGET(const char* server, int port, const char* path, char* result, int resultlength)
{
     ...
     unsigned long now;
     unsigned long apiTimeout = 15000;
     char resultLength = 0;
     String body = "";
     ...
     */
     ...
     Serial.println(body);
     //body.toCharArray(result, sizeof(result));
     // strncpy(result, body.c_str(), sizeof(result));
...
int InetGSM::httpPOST(const char* server, int port, const char* path, const char* parameters, char* result, int resultlength)
{
     ...
     gsm.SimpleWrite(server);
     gsm.SimpleWrite("\r\n");
     gsm.SimpleWrite("User-Agent: EnvisorV3.1\r\n");
